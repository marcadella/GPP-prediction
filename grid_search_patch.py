...
Patch of https://github.com/ploomber/sklearn-evaluation/blob/master/src/sklearn_evaluation/plot/grid_search.py
"""
...
def _grid_search_double(grid_scores, change, subset, cmap, ax, sort):
    ...
    try:
        row_names = sorted(set([t[0] for t in matrix_elements.keys()]), key=lambda x: (x[1] is None, x[1]))
    except:
        row_names = set([t[0] for t in matrix_elements.keys()])
    
    try:
        col_names = sorted(set([t[1] for t in matrix_elements.keys()]), key=lambda x: (x[1] is None, x[1]))
    except:
        col_names = set([t[1] for t in matrix_elements.keys()])
    ...
    x_label, _ = list(col_names)[0]
    y_label, _ = list(row_names)[0]
    ...
    def format_ticks(v):
        if isinstance(v, Number):
            ...
            return textwrap.fill(str(v), 16)
    
    # ticks for the axis
    row_labels = [format_ticks(x) for _, x in row_names]
    col_labels = [format_ticks(y) for _, y in col_names]
